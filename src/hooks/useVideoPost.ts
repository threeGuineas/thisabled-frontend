...
import { uploadVideo, waitForCaptionReady, publishPost, retryCaption, type Post, type AiStatus, type PostCategory } from '../services/posts'
...
export function useVideoPost() {
  ...
  const finishWithStatus = async (id: string, status: AiStatus, meta: Draft, startedAt = Date.now()): Promise<Post | null> => {
    ...
    if (status === 'processing') {
      ...
      if (Date.now() - startedAt < MAX_CAPTION_WAIT_MS) {
        return await finishWithStatus(id, 'processing', meta, startedAt)
      }
    ...
    const post = await publishPost(id, meta.category, meta.content, false)
    setPhase('idle')
    return post
  }

  const publish = async (file: File, durationSeconds: number, category: PostCategory, content: string): Promise<Post | null> => {
    setPhase('submitting')
    setError(null)
    const meta: Draft = { category, content }
    try {
      if (postId && draft) {
        // 자막 대기 타임아웃 뒤 재시도 — 같은 드래프트의 자막 상태만 다시 확인한다
        return await finishWithStatus(postId, 'processing', draft)
      }
      const uploaded = await uploadVideo(file, durationSeconds)
      setPostId(uploaded.post_id)
      setDraft(meta)
      return await finishWithStatus(uploaded.post_id, uploaded.caption_status, meta)
    } catch (err: unknown) {
  ...
  const publishWithoutCaption = async (): Promise<Post | null> => {
    ...
    setError(null)
    try {
      const post = await publishPost(postId, draft.category, draft.content, true)
      setPhase('idle')
      return post
    } catch (err: unknown) {
      setError(formatError(err, '게시에 실패했어요. 잠시 후 다시 시도해주세요.'))
