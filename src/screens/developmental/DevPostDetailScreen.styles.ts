...
const styles = {
  ...
  inputBar: [colors.bg.white, 'fixed bottom-0 left-0 right-0 flex items-center gap-2 px-4 py-3 shadow-[0_-2px_8px_rgba(0,0,0,0.04)]'].join(' '),
  textInput: [
    colors.bg.gray04, 'flex-1 rounded-full px-5 py-3.5 outline-none text-black placeholder:text-[#9898A8]',
    typography.base, typography.regular,
  ].join(' '),
  sendButton: 'w-12 h-12 rounded-full flex items-center justify-center flex-shrink-0',
  sendButtonActive: [colors.bg.green, 'w-12 h-12 rounded-full flex items-center justify-center flex-shrink-0'].join(' '),
  sendButtonInactive: [colors.bg.gray03, 'w-12 h-12 rounded-full flex items-center justify-center flex-shrink-0'].join(' '),
  sendIcon: 'w-5 h-5',
